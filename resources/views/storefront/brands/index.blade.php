...
<div class="px-4 py-6">
    ...
    <div class="mb-8">
        <div class="flex flex-wrap gap-2 justify-center">
            <a href="{{ route('storefront.brands.index') }}" 
            ...
            @foreach(range('A', 'Z') as $char)
                ...
                @endphp
                <a href="{{ route('storefront.brands.index', ['letter' => $char]) }}" 
            ...
            @if(in_array('#', $availableLetters))
                <a href="{{ route('storefront.brands.index', ['letter' => '#']) }}" 
                   class="px-3 py-1 rounded {{ ($letter === '#') ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300' }}">
                    #
                </a>
            @endif
            ...
            @if(isset($groupedBrands[$letter]) && $groupedBrands[$letter]->count() > 0)
                <div class="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6">
                    @foreach($groupedBrands[$letter] as $brand)
                        @include('storefront.brands._brand-card', ['brand' => $brand])
                    @endforeach
                </div>
            @else

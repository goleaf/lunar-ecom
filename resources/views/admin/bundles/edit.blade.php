...
(function() {
    ...
    const existing = @json($bundle->load(['items', 'prices']));
    ...
    function collectPrices() {
        return Array.from(pricesContainer.querySelectorAll('.price-row')).map(row => {
            const get = (field) => {
                const el = row.querySelector(`[data-field="${field}"]`);
                const val = el?.value?.trim();
                return val === '' ? null : val;
    ...
    function serializeForm() {
        ...
        const payload = {
            ...
            is_active: boolVal(form.querySelector('input[name="is_active"]')),
            is_featured: boolVal(form.querySelector('input[name="is_featured"]')),
            allow_customization: boolVal(form.querySelector('input[name="allow_customization"]')),
            show_individual_prices: boolVal(form.querySelector('input[name="show_individual_prices"]')),
            show_savings: boolVal(form.querySelector('input[name="show_savings"]')),
    ...
    form.addEventListener('submit', async (e) => {
        ...
        try {
            const res = await fetch(form.dataset.submitUrl, {
                method: form.dataset.method || 'PUT',
                headers: {

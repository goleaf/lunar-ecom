...
<div class="max-w-6xl mx-auto space-y-6">
    ...
    <div class="bg-white rounded-lg shadow p-6">
        ...
        <form id="recommendation-form" method="POST" action="{{ url()->current() }}" class="grid grid-cols-1 md:grid-cols-3 gap-4">
            @csrf
            <div>
                <label class="block text-xs text-slate-600 mb-1">Source product ID</label>
                <input type="number" name="source_product_id" class="w-full rounded border border-slate-300 px-3 py-2 text-sm">
            </div>
            <div>
                <label class="block text-xs text-slate-600 mb-1">Recommended product ID</label>
                <input type="number" name="recommended_product_id" class="w-full rounded border border-slate-300 px-3 py-2 text-sm" required>
            </div>
            <div>
            ...
            <tbody class="divide-y divide-slate-100">
                @forelse($rules as $rule)
                    <tr>
                        ...
                        <td class="px-4 py-3">{{ $rule->recommendedProduct?->translateAttribute('name') ?? 'Product' }}</td>
                        <td class="px-4 py-3">{{ $rule->priority }}</td>
                        <td class="px-4 py-3">{{ $rule->is_active ? 'Active' : 'Inactive' }}</td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="5" class="px-4 py-6 text-center text-slate-500">No recommendation rules found.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
...
recommendationForm?.addEventListener('submit', async (event) => {
    ...
    recommendationMessage.textContent = 'Saving...';

    const formData = new FormData(recommendationForm);
    const payload = Object.fromEntries(formData.entries());

    try {
        const response = await fetch(recommendationForm.action, {
            method: 'POST',
            headers: {
        ...
        recommendationMessage.textContent = data.message || 'Rule saved.';
        if (response.ok) {
            setTimeout(() => window.location.reload(), 800);
        }
    } catch (error) {
        recommendationMessage.textContent = 'Failed to save rule.';
    }
});
</script>
@endpush
